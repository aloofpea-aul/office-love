...
function generateRounds(workDate) {
  ...
  readTable('Rounds').forEach(function (r) {
    if (String(r.workDate) === workDate) existing[String(r.roundNo)] = true;
  });
  ...
  plans.forEach(function (p) {
    ...
    if (t.indexOf('T') > 0) t = t.split('T')[1];           // กันกรณีชีตแปลงเป็น Date
    var hh = parseInt(t.split(':')[0], 10);
    var mm = parseInt(t.split(':')[1] || '0', 10);
...
function apiBootstrap(me) {
  ...
  if (!readTable('Rounds').some(function (r) { return String(r.workDate) === workDate; }))
  ...
  var rounds = readTable('Rounds').filter(function (r) { return String(r.workDate) === workDate; })
    .sort(function (a, b) { return num(a.roundNo) - num(b.roundNo); })
    .map(function (r) {
  ...
  readTable('Shifts').forEach(function (s) {
    if (String(s.guardUserId) === me.userId && String(s.workDate) === workDate && !s.checkOutAt)
...
function apiSupervisorBoard(me, req) {
  ...
  var shifts = readTable('Shifts').filter(function (s) { return String(s.workDate) === workDate; })
    .map(function (s) {
